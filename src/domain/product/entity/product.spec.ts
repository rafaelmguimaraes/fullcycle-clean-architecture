...
describe("Product unit tests", () => {
  ...
  it("shuld throw error when id and name are empty", () => {
    expect(() => {
  ...
  it("shuld throw error when name is empty and when price is less than zero ", () => {
    expect(() => {
      const product = new Product("123", "", -1);
    }).toThrowError("product: Name is required,product: Price must be greater than zero");
  });

  it("shuld throw error when id and name are empty, and when price is less than zero ", () => {
    expect(() => {
